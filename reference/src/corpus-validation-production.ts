...
import { buildSparseLexicalIndex, tfIdfCosine, type SparseLexicalDocument } from "./sparse-lexical.js";
import type {
...
export async function validateProductionCorpus(
  ...
  const pairs = [...candidateKeys].sort();
  ...
  const semanticState = embeddingBackend
    ...
    : "pending";
  const bounded = candidateKeys.size <= Math.max(1, documents.length * policy.maximumCandidatesPerPage * 2);
  const validation = buildValidationReport(`production-corpus:${transaction.transactionHash}`, CORPUS_VALIDATION, [
    finding("corpus.lexical", lexicalState, `${documents.length} pages; ${candidateKeys.size} bounded LSH/posting candidates; threshold=${policy.lexicalDuplicateThreshold}`),
    ...
    finding("corpus.scale", bounded && documents.length > 0 ? "pass" : "fail", `${documents.length} pages validated with ${candidateKeys.size} candidates; cap=${documents.length * policy.maximumCandidatesPerPage * 2}`),
  ]);
  const canonical = {
...
function boundedLexicalCandidates(index: ReturnType<typeof buildSparseLexicalIndex>, policy: ProductionCorpusValidationPolicy): Set<string> {
  ...
  for (const document of index.documents) {
    ...
    for (const { term } of rareTerms) {
      for (const otherId of index.postings[term] ?? []) {
        if (otherId === document.id || local.size >= policy.maximumCandidatesPerPage) continue;
        local.add(otherId);
      }
    }
    perPage.set(document.id, local);
  }
  const lexicalBuckets = new Map<string, string[]>();
  for (const document of index.documents) {
    const signatures = lexicalBandSignatures(document, policy.lexicalBands);
    signatures.forEach((signature, band) => {
      const key = `${band}:${signature}`;
      const ids = lexicalBuckets.get(key) ?? [];
      ids.push(document.id);
      lexicalBuckets.set(key, ids);
    });
  }
  for (const ids of lexicalBuckets.values()) addBucketPairs(ids, perPage, policy.maximumCandidatesPerPage);
  for (const [id, local] of perPage) {
    for (const otherId of local) {
      if (!byId.has(otherId)) continue;
      candidates.add(pairKey(id, otherId));
    }
  }
  return candidates;
}
...
function addSemanticLshCandidates(
  ...
  for (const pageId of pageIds) {
    ...
    for (let band = 0; band < policy.semanticBands; band += 1) {
      ...
      for (let offset = 0; offset < Math.min(16, width); offset += 1) {
        const index = Math.min(vector.length - 1, band * width + offset);
        if (vector[index] >= 0) signature |= 1 << offset;
      }
  ...
  for (const [id, local] of perPage) for (const otherId of local) candidates.add(pairKey(id, otherId));
}
...
function hasInformationAndEvidence(transaction: PageDraftTransaction, policy: ProductionCorpusValidationPolicy): boolean {
  ...
  return transaction.source.pages.every((page) => {
    ...
    for (const moduleId of page.moduleIds) {
      ...
      for (const claimId of module.claimIds) for (const evidenceId of required(claimById, claimId).evidenceIds) evidenceIds.add(evidenceId);
...
function validatePolicy(policy: ProductionCorpusValidationPolicy): void {
  if (![policy.lexicalDuplicateThreshold, policy.semanticDuplicateThreshold].every((value) => Number.isFinite(value) && value >= 0 && value <= 1)) throw new Error("duplicate thresholds must be within [0,1]");
