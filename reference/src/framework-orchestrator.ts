...
export interface FrameworkGroundworkInput {
  project: ProjectInput;
  manifest: FrameworkManifest;
  contextCorpus: { id: string; version: string; cases: readonly ContextCorpusCase[]; splitPolicy: SplitPolicy; generationAgentId?: string };
  calibrationObservations: readonly CalibrationObservation[];
  candidateSeeds: readonly CandidatePageSeed[];
  selection: CorpusSelectionOptions;
  claims?: { allowed?: readonly string[]; prohibited?: readonly string[] };
}
...
export function prepareFrameworkProject(input: FrameworkGroundworkInput): PreparedFrameworkProject {
  ...
  for (const coordinate of coordinates) assertValidationPass(coordinate.validation);
  const selectedCorpus = selectCorpusPlan(project, contextCorpus, coordinates, input.selection);
  assertValidationPass(selectedCorpus.validation);
  assertValidationPass(selectedCorpus.typedGraph.validation);
  const generationJobs = compilePageGenerationJobs(project, selectedCorpus, {
    ...(input.claims?.allowed === undefined ? {} : { allowedClaimIds: input.claims.allowed }),
    ...(input.claims?.prohibited === undefined ? {} : { prohibitedClaimIds: input.claims.prohibited }),
  });
  for (const job of generationJobs) assertValidationPass(job.validation);
  const checkpointHashes = {
    project: project.projectHash,
    corpus: contextCorpus.corpusHash,
    calibration: calibration.model.modelHash,
    coordinates: sha256(JSON.stringify(coordinates.map((item) => [item.id, item.candidateHash]))),
    selectedCorpus: selectedCorpus.planHash,
    generationJobs: sha256(JSON.stringify(generationJobs.map((item) => [item.jobId, item.jobHash]))),
  };
  const validationReports = [project.validation, contextCorpus.validation, calibration.validationReport, ...coordinates.map((item) => item.validation), selectedCorpus.validation, selectedCorpus.typedGraph.validation, ...generationJobs.map((item) => item.validation)];
  const canonical = { projectId: project.input.id, checkpointHashes, validationHashes: validationReports.map((item) => item.reportHash) };
...
export async function executeFrameworkProject(
  prepared: PreparedFrameworkProject,
  executor: AgentPassExecutor,
  options: { maximumRepairAttempts?: number } = {},
): Promise<ExecutedFrameworkProject> {
  const generationRuns: GenerationRunResult[] = [];
  for (const job of prepared.generationJobs) generationRuns.push(await runPageGenerationJob(job, executor, { maximumRepairAttempts: options.maximumRepairAttempts ?? 1 }));
  const preview = buildFrameworkPreview(prepared.project, prepared.contextCorpus, prepared.selectedCorpus, prepared.generationJobs, generationRuns, [prepared.calibration.validationReport, prepared.selectedCorpus.typedGraph.validation]);
