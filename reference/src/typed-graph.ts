...
export function compileTypedGraph(pageIds: readonly string[], edges: readonly TypedGraphEdge[]): TypedPageGraph {
  const knownPages = new Set(pageIds); if (knownPages.size !== pageIds.length) throw new Error("duplicate page IDs in graph");
  const edgeIds = new Set<string>();
  const canonicalEdges = [...edges].map((edge) => {
    if (!edge.id.trim() || edgeIds.has(edge.id)) throw new Error(`invalid or duplicate graph edge ${edge.id}`); edgeIds.add(edge.id);
  ...
  const outgoing: Record<string, string[]> = Object.fromEntries([...pageIds].sort().map((id) => [id, []]));
  const incoming: Record<string, string[]> = Object.fromEntries([...pageIds].sort().map((id) => [id, []]));
  for (const edge of canonicalEdges) { outgoing[edge.fromPageId].push(edge.id); incoming[edge.toPageId].push(edge.id); }
  for (const ids of Object.values(outgoing)) ids.sort(); for (const ids of Object.values(incoming)) ids.sort();
  const orphanIds = [...knownPages].filter((id) => outgoing[id].length === 0 && incoming[id].length === 0).sort();
  const findings = [finding("graph.identity", "pass", `${canonicalEdges.length} unique typed edges compiled for ${knownPages.size} pages`), finding("graph.semantics", "pass", "all edges include type, rationale, provenance, eligibility, and priority"), finding("graph.path-utility", orphanIds.length === 0 ? "pass" : "pending", orphanIds.length === 0 ? "no orphan pages" : `orphan review required for ${orphanIds.join(",")}`)];
...
export function findTypedPaths(graph: TypedPageGraph, fromPageId: string, predicate: (pageId: string) => boolean, maximumDepth = 6): string[][] {
  if (!graph.pageIds.includes(fromPageId)) throw new Error(`unknown start page ${fromPageId}`);
  const edgeById = new Map(graph.edges.map((edge) => [edge.id, edge])); const queue: { pageId: string; path: string[] }[] = [{ pageId: fromPageId, path: [fromPageId] }]; const output: string[][] = [];
  while (queue.length > 0) { const current = queue.shift()!; if (current.path.length > 1 && predicate(current.pageId)) output.push(current.path); if (current.path.length > maximumDepth) continue; for (const edgeId of graph.outgoing[current.pageId] ?? []) { const edge = edgeById.get(edgeId); if (!edge || current.path.includes(edge.toPageId)) continue; queue.push({ pageId: edge.toPageId, path: [...current.path, edge.toPageId] }); } }
