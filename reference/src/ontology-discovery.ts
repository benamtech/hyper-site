...
function hasLabelDuplicateEvidence(left: ApprovedOntologyAttribute, right: ApprovedOntologyAttribute): boolean {
  ...
  const prefixContainment = left.normalizedLabel.startsWith(`${right.normalizedLabel}-`) || right.normalizedLabel.startsWith(`${left.normalizedLabel}-`);
  return jaccard >= 0.8 || prefixContainment;
}
