...
export function compileSite(source: SiteSource): CompiledSite {
  ...
  const pages: PageIR[] = [...source.pages].sort((a, b) => a.id.localeCompare(b.id)).map((page) => {
    ...
    return {
      ...
      dependencyIds: uniqueSorted([page.id, ...page.moduleIds, ...modules.flatMap((module) => module.sourceIds)]),
    };
  });
  ...
  return { pages: artifacts, sitemapXml, dependencyIndex, buildHash };
}
