...
export interface BrowserConfigurationInput { packageJsonText?: string; browserslistRcText?: string; frameworkConfigs?: Partial<Record<"vite" | "next" | "astro", string>>; }
export interface BrowserVersionFloor { chrome?: number; edge?: number; firefox?: number; safari?: number; iosSafari?: number; }
export interface BrowserTargetReport { source: BrowserConfigSource; queries: string[]; tier: BrowserTier; explicit: boolean; floors: BrowserVersionFloor; rationale: string[]; reportId: string; }

export const AMTECH_TIER_2_QUERIES = [
  "Chrome >= 111", "Edge >= 111", "Firefox >= 113", "Safari >= 16.4", "ios_saf >= 16.4", "not dead", "not op_mini all",
] as const;
...
export function resolveBrowserTargets(input: BrowserConfigurationInput): BrowserTargetReport {
  ...
  return buildReport("default", [...AMTECH_TIER_2_QUERIES], false, ["No browser configuration was found; the framework selected its conservative-modern Tier 2 baseline."]);
}
...
function buildReport(source: BrowserConfigSource, queries: string[], explicit: boolean, extraRationale: string[] = []): BrowserTargetReport {
  ...
  return { source, queries: normalized, tier, explicit, floors, rationale: [...extraRationale, `Resolved ${normalized.length} browser queries from ${source}.`, `Mapped targets to Tier ${tier}.`], reportId: stableId(`${source}\0${normalized.join("\0")}\0${tier}`) };
}
function readPackageQueries(text?: string): string[] {
  ...
  } catch { throw new Error("package.json is not valid JSON while resolving browserslist"); }
  return [];
}
function readRcQueries(text?: string): string[] { return !text?.trim() ? [] : text.split(/\r?\n/).map((line) => line.replace(/#.*/, "").trim()).filter(Boolean); }
function extractFrameworkQueries(text: string): string[] { return [...text.matchAll(/(?:build\.target|target|browserslist)\s*[:=]\s*["'`]([^"'`]+)["'`]/g)].map((match) => match[1]).filter(Boolean); }
...
function extractFloors(queries: readonly string[]): BrowserVersionFloor {
  ...
  for (const query of queries) {
    ...
    switch (match[1].toLowerCase()) {
      ...
      case "ios_saf": case "ios": floors.iosSafari = version; break;
    }
  }
  return floors;
}
function meetsFloors(actual: BrowserVersionFloor, expected: Required<BrowserVersionFloor>): boolean {
  return (actual.chrome ?? 0) >= expected.chrome && (actual.edge ?? 0) >= expected.edge && (actual.firefox ?? 0) >= expected.firefox && (actual.safari ?? 0) >= expected.safari && (actual.iosSafari ?? 0) >= expected.iosSafari;
}
function stableId(value: string): string { let hash = 0x811c9dc5; for (let index = 0; index < value.length; index += 1) { hash ^= value.charCodeAt(index); hash = Math.imul(hash, 0x01000193); } return `browser-${(hash >>> 0).toString(16).padStart(8, "0")}`; }
